class PossibleTrips {
  ...
  distanceOfTrips() {
    ...
    return trips.filter(trip => trip.endPoint == endPoint).sort(compare(a, b))[0].map(trip => trip.distance).reduce(reducer);
  }
    ...
    traks.forEach(function (track) {
      ...
      do {
        ...
        if (minStops < maxStops || minStops == 0 || minStips > maxStops) {
          if (lazyStopCondition(trips[tripNumber])) {
            break;
          };
        } else {
          if (exactlyMaxStopsCondition(trips[tripNumber])) {
            break;
          };
        }
      } while (1 == 1);
  ...
  possibleStartTracks(point = null) {
    return this.trackFinder.findByPoint(point || startPoint).filter(track => track.trips < track.possibleTrips);
  }
}